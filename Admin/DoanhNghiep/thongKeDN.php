...
if(isset($_POST['btnExport'])){
	...
	$sheet = $objExcel->getActiveSheet()->setTitle('Danh sách doanh nghiệp');
	//xem nguoi dung o dong nao
	$rowCount = 1;
	$sheet->setCellValue('A'.$rowCount, 'Mã doanh nghiệp');
	$sheet->setCellValue('B'.$rowCount, 'Mã người dùng');
	$sheet->setCellValue('C'.$rowCount, 'Tên doanh nghiệp');
	$sheet->setCellValue('D'.$rowCount, 'Địa chỉ');
	
//	data tra ve
	$result = mysqli_query($conn, "SELECT * FROM tbl_doanhnghiep WHERE ma_nguoidung IS NOT NULL");
	
//	set du lieu tra ve
	while($row = mysqli_fetch_array($result)){
		$rowCount++;
		$sheet->setCellValue('A'.$rowCount, $row['madn']);
  ...
  <nav class="main-header navbar navbar-expand navbar-white navbar-light">
    ...
    <ul class="navbar-nav">
      ...
      <li class="nav-item d-none d-sm-inline-block">
        <a href="../../indexadmin.php" class="nav-link">Trang chủ</a>
      </li>
    </ul>
...
<!--
                <a class="btn btn-primary btn-xs" href="../../forms/themDN.html" style=" margin-left: .875em;">
                  <i class="fas fa-plus">
                  </i>
                  Thêm
                </a>
              ...
              <div class="card-body" id="dn_table">
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tr>
					...
					<?php
					  include("doanhnghiep-select.php");
					  ?>
